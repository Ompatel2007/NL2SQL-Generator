...
export function HelpView({ onBackToWorkspace }: HelpViewProps) {
  ...
  const manualSections = [
    ...
    {
      id: 12,
      title: "Understanding Theory / Relational Algebra",
      whatItDoes: "Provides academic documentation on relational algebra operators, SQL sublanguages, and formal mathematical notation.",
      whatToDo: "Click the 'Theory' tab at the top of the center canvas.",
      controls: "The 'Theory' tab button.",
      processing: "Renders academic summaries covering DQL, DML, DDL, selection (σ), projection (π), joins (⋈), aggregation (γ), and ACID transaction guarantees.",
    ...
    },
    {
      ...
      whatItDoes: "Displays algorithmic time complexity metrics for database operations (e.g. $O(N)$ scans, $O(N \\times M)$ nested joins, $O(N \\log N)$ external sorts).",
      whatToDo: "Look at the right sidebar ('Execution Theory & Algebra') while inspecting any pipeline step.",
      controls: "The 'Relational Notation / Semantics' card in the Explanation Panel.",
      processing: "Maps the active operator to its formal relational algebraic formula and big-O computational upper bound.",
      expectedOutput: "Mathematical expressions and operational complexity notes explaining why specific database operators perform with certain efficiencies.",
    },
    {
      id: 14,
